...
router.get('/profile/:username', optionalAuth, asyncHandler(async (req, res) => {
  try {
    ...
    // Get user's canvases directly
    const userCanvases = await Canvas.find({ 
      owner: user._id, 
      isActive: true 
    }).select('title thumbnail createdAt settings.isPublic views likes').sort({ createdAt: -1 });

    // Get collaborations directly
    ...
    let visibleCanvases = userCanvases;
    
    // If not the owner, only show public canvases
    if (!req.user || req.user._id.toString() !== user._id.toString()) {
      visibleCanvases = userCanvases.filter(canvas => canvas.settings.isPublic);
    }
    ...
    const stats = {
      ...
      totalLikes: visibleCanvases.reduce((sum, canvas) => sum + canvas.likes.length, 0),
